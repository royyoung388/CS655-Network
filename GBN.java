...
public class GBN extends NetworkSimulator {
    /*
...
//        System.out.printf("%d, %d\n", firstACK, ack);
        ...
        if (firstACKTime.get(index) == 0)
            firstACKTime.set(index, getTime());

        // sack
        for (int s : packet.getSack()) {
            int sindex = s;
            if (sindex < firstACK)
                sindex += LimitSeqNo;
            if (sindex - firstACK >= WindowSize)
                break;
            sindex = sindex - firstACK + aLAR;
            aBuffer.get(sindex).setAcknum(s);

            if (firstACKTime.get(sindex) != 0)
                firstACKTime.set(sindex, getTime());
        }
        ...
        if (index == aLAR) {
            ...
            // slide window
            aLAR = index;
            // send next packets, skip sack
            for (int i = aLPS + 1; i <= aLAR + WindowSize; i++) {
                if (i >= aBuffer.size())
                    break;
                Packet p = aBuffer.get(i);

                // skip sack, until next unACK packet.
                if (p.getAcknum() != p.getSeqnum()) {
                    aSendPacket(i);
                }
            }
        }
    ...
    protected void aTimerInterrupt() {
        // retransmit all packet which ackNo != seqNo (unACK)
...
//        System.out.printf("%d, %d\n", firstSeq, seq);
        ...
        System.out.printf("bNPE: %d, index: %d, expectSeq: %d\n", bNPE, index, expectSeq);
...
//        System.out.printf("%d, %d\n", bNPE, aBuffer.size());
        ...
        if (bNPE != 0) {
            ...
            while (sack.size() < 5) {
                if (idx >= bBuffer.length || idx - bNPE >= WindowSize) {
                    break;
                }
                if (bBuffer[idx] != null)
                    sack.add(idx % LimitSeqNo);
                idx += 1;
            }
            cumulate.setSack(sack.stream().mapToInt(i -> i).toArray());
            cumulate.setChecksum(checkSum(cumulate));
            toLayer3(1, cumulate);
        }
    }
